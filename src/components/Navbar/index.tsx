  ...
  return (
    ...
    >
      <div className="flex h-24 items-center justify-between px-7 lg:px-14">
        <Link href="/" className="text-purple-500" scroll={false}>
          <h1
        ...
        <div className="flex lg:hidden">
          ...
          >
            {!isOpen ? (
              <BiMenuAltRight className="h-10 w-10 fill-current text-purple-500" />
            ) : (
              <BiX className="h-10 w-10 fill-current text-purple-500" />
            )}
          </button>
        </div>
        ...
        <div className="text-base-content hidden  gap-10 lg:flex">
          {NAVBAR_ITEMS.map((item, index) => (
            <Link
              href={item.href}
              className="flex flex-col items-end font-mono text-sm  text-primary transition-all duration-300 hover:text-purple-400 xl:text-base"
              key={index}
              data-aos="fade-down"
              ...
              scroll={false}
            >
              <span className="text-xs text-purple-500">{item.number}</span>
              <p>{`// ${item.title}`}</p>
            </Link>
          ))}
        </div>
        ...
        <div className="fixed right-0 top-0 z-[99] h-full w-3/4 bg-base_col_darker drop-shadow-lg md:w-1/2">
          <div className="mt-44 flex flex-col items-center justify-center gap-12 text-base md:text-lg">
            {NAVBAR_ITEMS.map((item, index) => (
              <Link
                href={item.href}
                className="text-center font-mono text-primary hover:text-purple-400"
                key={index}
                onClick={() => setIsOpen(false)}
              >
                <p className="flex flex-col">
                  <span className="text-sm text-purple-500">{item.number}.</span>
                  {`${item.title}`}
                </p>
              </Link>
            ))}
